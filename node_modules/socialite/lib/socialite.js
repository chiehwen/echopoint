...
function Api(method, url, postData, callback) {
  ...
  //if (typeof postData.oauth !== 'undefined') 
    //this.options.oauth = postData.oauth
...
Api.prototype.cleanUrl = function(url) {
  url = url.trim();
  ...
  if (accessToken) {
    url += ~url.indexOf('?') ? '&' : '?';
    url += 'access_token=' + accessToken + '&oauth_token=' + accessToken;
  }
...
Api.prototype.post = function() {
  ...
  this.options.uri = this.options.uri + postData;
  //this.options.json  = this.postData;
...
//self.callback(null, {bod: body, response: res, opts: self.options});
    self.end(body);
  });
 ...
 *
 *      api.get("zuck?fields=picture", callback)
 *
 *    OR
 *
 *      var params = { fields: picture };
 *      api.get("zuck", params, callback);
 *
 *    Api calls that redirect directly to an image
 *    will return a json response with relavant fields
 *
 *      api.get("/zuck/picture", callback);
 *
 *      {
 *        image: true,
...
post: function (url, postData, callback) {
  ...
  return new Api('POST', url, postData, callback);
},
 ...
 */
getOauthUrl: function (type, params, opts) {
  if(typeof type !== 'string') {
    params = type;
    type = 'auth';
  }

  var url = apiConfig[api].dialog[type];
...
authorize: function (method, path, params, callback) {
  var self = this;

  this[method](path, params, function(err, res) {
    if (!err) 
